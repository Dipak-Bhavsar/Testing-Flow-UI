...
import dynamicBlockSchema from "../../../dummyData/workflowDynamicBlockSchema";
...
// State for block form values
const blockValues = reactive({});

// Watch for changes in blockValues and update valueJson store
watch(
    blockValues,
    (newValues) => {
...
const dynamicInputs = dynamicBlockSchema;
...
<template>
    <div class="bg-white p-2 rounded-lg w-full max-w-lg flex flex-col gap-1 border border-slate-200 shadow-xl max-h-96 overflow-y-auto">
        <div class="flex justify-end">
            <button @click="removeNodes([props.id])" class="text-slate-900 text-end hover:text-rose-600">
                <span class="p-1 text-base"><i class="hgi-stroke hgi-delete-02"></i></span>
            </button>
        </div>
